...
<div class="max-w-7xl mx-auto space-y-8 animate-fade-in pb-20">
 ...
 <div>
 <h1 class="text-2xl font-bold text-white tracking-tight">Inventory Management</h1>
 <p class="text-sm text-slate-400 mt-1">Manage hospital stock, medical supplies, and vendor relations.</p>
 </div>
 <div class="mt-4 md:mt-0">
 <button onclick="document.getElementById('addStockModal').classList.remove('hidden')" class="px-5 py-2.5 bg-sage hover:bg-sage text-white text-sm font-bold rounded-lg /10 transition-colors">
 Receive New Stock
 </button>
 </div>
 </header>
 ...
 return "<a href=\"".route('operations.supply.warehouse', ['subtab' => $sub])."\" class=\"pb-4 text-sm font-bold border-b-2 transition-all {$cls}\">{$label}</a>";
 };
 ?>
 ...
 <p class="text-xs font-bold text-slate-500 font-medium mb-2">Total SKUs</p>
 <h3 class="text-3xl font-bold text-white"><?php echo e(number_format($stats['total_items'])); ?></h3>
 </div>
 <div class="bg-[#2a2e38] rounded-xl p-6 border border-subtle ">
 <p class="text-xs font-bold text-slate-500 font-medium mb-2">Inventory Value</p>
 <h3 class="text-3xl font-bold text-emerald-500">XAF <?php echo e(number_format($stats['total_value'])); ?></h3>
 </div>
 <div class="bg-[#2a2e38] rounded-xl p-6 border border-subtle ">
 <p class="text-xs font-bold text-slate-500 font-medium mb-2">Pending Requests</p>
 <h3 class="text-3xl font-bold text-sage"><?php echo e($stats['pending_reqs']); ?></h3>
 </div>
 <div class="bg-[#2a2e38] rounded-xl p-6 border border-subtle ">
 <p class="text-xs font-bold text-rose-500 font-medium mb-2">Low Stock Alerts</p>
 ...
 <!-- Blood Bank Snapshot -->
 <div class="space-y-4">
 <h3 class="text-sm font-bold text-slate-400 font-medium px-2">Blood Bank Reserves</h3>
 ...
 <div class="bg-[#2a2e38] p-4 rounded-xl border border-subtle text-center">
 <span class="text-xs font-bold text-slate-500 block mb-1"><?php echo e($b->blood_group); ?></span>
 <div class="text-xl font-bold <?php echo e($b->units_available < 10 ? 'text-rose-500' : 'text-white'); ?>"><?php echo e($b->units_available); ?></div>
 <span class="text-[12px] text-slate-600 font-medium uppercase">Units</span>
 </div>
 ...
 <div class="px-6 py-4 border-b border-subtle bg-card/40">
 ...
 <td class="px-6 py-4">
 <span class="px-2 py-1 rounded text-[12px] font-bold <?php echo e($a->movement_type === 'IN' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'); ?>">
 ...
 <tr class="hover:bg-slate-700/30 transition-colors">
 <td class="px-6 py-4">
 <div class="text-white font-bold"><?php echo e($s->item_name); ?></div>
 <div class="text-[12px] text-slate-500 font-medium mt-0.5 uppercase">Batch: <?php echo e($s->batch_number ?: 'N/A'); ?></div>
 </td>
 ...
 <tr class="hover:bg-slate-700/30 transition-colors">
 <td class="px-6 py-4">
 <div class="text-white font-bold uppercase text-xs"><?php echo e($r->pharmacy_item_name); ?></div>
 <div class="text-[12px] text-slate-500 font-medium mt-0.5">QTY: <?php echo e($r->requested_qty); ?></div>
 </td>
 <td class="px-6 py-4 text-slate-400"><?php echo e($r->requester_name); ?></td>
 <td class="px-6 py-4 text-center">
 <span class="px-2.5 py-1 rounded-full text-[12px] font-bold uppercase
 <?php if($r->status === 'completed'): ?> bg-emerald-500/10 text-emerald-500
 <?php elseif($r->status === 'dispatched'): ?> bg-sage/10 text-sage
 ...
 <button onclick="openDispatchModal('<?php echo e($r->id); ?>', '<?php echo e($r->pharmacy_item_name); ?>', <?php echo e($r->requested_qty); ?>)" class="px-4 py-2 bg-sage hover:bg-sage text-white text-[12px] font-bold rounded-lg transition-all">Approve Dispatch</button>
 <?php else: ?>
 <span class="text-xs text-slate-500 ">Fulfilled</span>
 <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
 </td>
 </tr>
 <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
 ...
 <div class="lg:col-span-2 bg-[#2a2e38] rounded-xl border border-subtle overflow-hidden">
 <div class="px-6 py-4 border-b border-subtle bg-card/40">
 ...
 <div class="text-[12px] text-slate-500 font-medium mt-0.5">Contact: <?php echo e($v->contact_person ?? 'N/A'); ?></div>
 </td>
 <td class="px-6 py-4 text-center text-slate-300 font-bold"><?php echo e($v->item_count ?? 0); ?></td>
 <td class="px-6 py-4 text-center">
 <span class="px-2 py-0.5 rounded text-[12px] font-bold uppercase <?php echo e(($v->status ?? 'active') === 'active' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'); ?>">
 ...
 <!-- Add Vendor -->
 <div class="bg-[#2a2e38] rounded-xl border border-subtle p-6 h-fit">
 <h3 class="text-sm font-bold text-slate-200 mb-6">Register New Supplier</h3>
 <form method="POST" action="<?php echo e(route('operations.supply.warehouse.action')); ?>" class="space-y-4">
 <?php echo csrf_field(); ?>
 ...
 <input type="text" name="name" required class="w-full bg-card border border-slate-700 rounded-lg px-4 py-2 text-sm text-slate-200 outline-none focus:ring-2 focus:ring-blue-600" placeholder="e.g. PharmaCorp Ltd">
 </div>
 <div>
 <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Contact Person</label>
 <input type="text" name="contact_person" class="w-full bg-card border border-slate-700 rounded-lg px-4 py-2 text-sm text-slate-200 outline-none focus:ring-2 focus:ring-blue-600">
 </div>
 <div>
 <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Email / Phone</label>
 <input type="text" name="email" class="w-full bg-card border border-slate-700 rounded-lg px-4 py-2 text-sm text-slate-200 outline-none focus:ring-2 focus:ring-blue-600">
 </div>
...
<!-- Modal: Add Stock -->
<div id="addStockModal" class="fixed inset-0 bg-card/80 z-[100] hidden flex items-center justify-center p-6">
 <div class="bg-[#2a2e38] w-full max-w-lg rounded-xl p-8 border border-subtle">
 <h3 class="text-xl font-bold text-white mb-6">Stock Intake Entry</h3>
 <form method="POST" action="<?php echo e(route('operations.supply.warehouse.action')); ?>" class="space-y-5">
 <?php echo csrf_field(); ?>
 <input type="hidden" name="action" value="add_stock">
 <div>
 <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Item Name</label>
 <input type="text" name="item_name" required placeholder="e.g. Paracetamol 500mg" class="w-full bg-card border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-slate-200 outline-none focus:ring-2 focus:ring-blue-600">
 </div>
 <div class="grid grid-cols-2 gap-4">
 <div>
 <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Category</label>
 <input type="text" name="category" required class="w-full bg-card border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-slate-200 outline-none focus:ring-2 focus:ring-blue-600">
 </div>
 <div>
 <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Quantity</label>
 <input type="number" name="bulk_quantity" required class="w-full bg-card border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-slate-200 outline-none focus:ring-2 focus:ring-blue-600">
 </div>
 </div>
 ...
 <button type="submit" class="flex-1 py-3 bg-sage hover:bg-sage text-white rounded-lg text-sm font-bold transition-all /20">Save Stock Entry</button>
 </div>
 </form>
 </div>
</div>
