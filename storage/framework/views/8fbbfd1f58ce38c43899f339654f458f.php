...
<div class="max-w-7xl mx-auto space-y-8 animate-fade-in pb-20">
    ...
    <div class="flex flex-col md:flex-row items-center gap-6">
        <div class="flex bg-slate-800 rounded-lg border border-slate-700/60 p-1 shadow-sm">
            <a href="<?php echo e(route('appointments', ['date' => \Carbon\Carbon::parse($date)->subDay()->toDateString()])); ?>" class="p-3 text-slate-400 hover:text-blue-400 transition-colors">
            ...
            <a href="<?php echo e(route('appointments', ['date' => \Carbon\Carbon::parse($date)->addDay()->toDateString()])); ?>" class="p-3 text-slate-400 hover:text-blue-400 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"/></svg>
            </a>
        </div>
        <div class="relative">
            <input type="date" value="<?php echo e($date); ?>" onchange="location.href='?date='+this.value" class="bg-slate-800 border border-slate-700/60 rounded-lg px-6 py-2.5 text-xs font-bold uppercase tracking-wider text-slate-200 outline-none focus:ring-2 focus:ring-blue-600 transition-all cursor-pointer">
        </div>
    </div>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($requests->isNotEmpty()): ?>
        <div class="bg-slate-800 rounded-xl p-8 border border-amber-500/20 bg-gradient-to-br from-amber-500/5 to-transparent relative overflow-hidden">
            <h3 class="text-[10px] font-bold text-amber-500 uppercase tracking-[0.2em] mb-8">Pending Portal Requests (<?php echo e($requests->count()); ?>)</h3>
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $requests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $req): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                    <div class="bg-slate-900/40 p-6 rounded-lg border border-slate-700/60 hover:border-amber-500/30 transition-all group">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <h4 class="text-xs font-bold text-slate-100 uppercase tracking-tight group-hover:text-amber-400 transition-colors"><?php echo e($req->full_name); ?></h4>
                                <p class="text-[9px] text-slate-500 font-bold uppercase mt-1"><?php echo e($req->medical_id); ?></p>
                            </div>
                            <span class="px-2 py-0.5 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded text-[9px] font-bold uppercase"><?php echo e($req->urgency); ?></span>
                        </div>
                        <p class="text-[11px] text-slate-400 italic mb-6 border-l-2 border-amber-500/20 pl-4">"<?php echo e($req->reason); ?>"</p>
    ...
    <div class="bg-slate-800 rounded-xl border border-slate-700/60 shadow-sm overflow-hidden">
        <div class="px-8 py-5 border-b border-slate-700/60 bg-slate-900/40 flex justify-between items-center">
            ...
            <span class="text-[10px] font-bold text-blue-400 uppercase tracking-wider">Confirmed Today: <?php echo e($appointments->where('status', 'confirmed')->count()); ?></span>
        </div>
                ...
                <tbody class="divide-y divide-slate-700/40">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $appointments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr class="hover:bg-slate-700/30 transition-colors">
                            ...
                            <td class="px-8 py-6">
                                <div class="font-bold text-slate-100 uppercase text-xs"><?php echo e($a->full_name); ?></div>
                                <div class="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5"><?php echo e($a->appointment_number); ?></div>
                            </td>
                            <td class="px-8 py-6">
                                <div class="text-xs font-bold text-slate-300 uppercase tracking-tight"><?php echo e($a->doctor_name ?? 'Not Assigned'); ?></div>
                                <div class="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1"><?php echo e($a->dept_name ?? 'General Unit'); ?></div>
                            </td>
                            <td class="px-8 py-6 text-center">
                                ...
                                ?>
                                <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase border <?php echo e($cls); ?>">
                                    <?php echo e($a->status); ?>
        ...
        <form method="POST" action="<?php echo e(route('appointments.store')); ?>" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="date" required value="<?php echo e($date); ?>" class="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-3 text-sm font-bold text-slate-200 outline-none focus:ring-2 focus:ring-blue-600 transition-all">
                </div>
                <div>
