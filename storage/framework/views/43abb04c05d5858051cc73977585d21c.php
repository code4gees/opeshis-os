...
<?php $__env->startSection('title', 'Patient Dossier — ' . $patient->full_name); ?>
...
<div class="space-y-8 animate-fade-in pb-20">
    ...
    <header class="flex justify-between items-center mb-10 pb-8 border-b border-white/10">
        <div class="flex items-center gap-10">
            ...
            <div>
                <h1 class="text-4xl font-black text-white tracking-tighter uppercase italic leading-none"><?php echo e($patient->full_name); ?></h1>
                <div class="flex items-center gap-6 mt-4">
                    <span class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] bg-indigo-500/10 border border-indigo-500/20 px-4 py-1.5 rounded-xl shadow-lg shadow-indigo-500/5"><?php echo e($patient->medical_id); ?></span>
                    <span class="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] italic"><?php echo e($patient->gender); ?> · <?php echo e(\Carbon\Carbon::parse($patient->date_of_birth)->age); ?> Years</span>
                    <span class="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em] bg-rose-500/10 border border-rose-500/20 px-4 py-1.5 rounded-xl shadow-lg shadow-rose-500/5 italic">BLOOD: <?php echo e($patient->blood_group); ?></span>
                </div>
            </div>
        </div>
        <div class="flex gap-4">
            ...
            <a href="<?php echo e(route('emr', ['queue_id' => $patient->id])); ?>" class="px-8 py-4 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 transition-all border border-indigo-500/50">
        ...
        <div class="lg:col-span-1 space-y-10">
            <div class="glass-panel p-10 rounded-[3rem] border border-white/10 shadow-xl bg-white/[0.02]">
                ...
                <div class="space-y-8">
                    <div>
                        ...
                        <p class="text-sm font-black text-rose-400 uppercase leading-relaxed"><?php echo e($patient->allergies ?: 'NO_KNOWN_ALLERGIES_IDENTIFIED'); ?></p>
                    </div>
                    <div>
                        <p class="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mb-3 italic">Genotype Identification</p>
                        <p class="text-sm font-black text-white uppercase italic"><?php echo e($patient->genotype ?: 'NULL_SPEC'); ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-panel p-10 rounded-[3rem] border border-white/10 shadow-xl bg-slate-900 relative overflow-hidden">
                <div class="relative z-10">
                    ...
                    <?php $currentAdmission = $admissions->where('status', 'admitted')->first(); ?>
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($currentAdmission): ?>
                        <div class="p-6 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl shadow-lg shadow-emerald-500/5 animate-pulse">
                            ...
                            <p class="text-sm font-black text-white mt-2 uppercase italic tracking-tighter">NODE: <?php echo e($currentAdmission->room_no ?? 'ICU_CORE_201'); ?></p>
                        </div>
                    <?php else: ?>
                        ...
                        <div class="relative pl-24 group">
                            ...
                            <div class="flex justify-between items-center mb-6">
                                <div>
                                    ...
                                    <p class="text-[9px] font-bold text-slate-500 mt-1 uppercase tracking-[0.2em] italic">Physician Node: <?php echo e($e->doctor_name); ?></p>
                                </div>
                                <span class="text-[11px] font-black text-slate-500 uppercase tracking-widest italic leading-none"><?php echo e(\Carbon\Carbon::parse($e->created_at)->format('d M Y')); ?></span>
                            </div>
                            <div class="p-10 bg-white/5 rounded-[2.5rem] border border-white/5 space-y-6 shadow-inner group-hover:bg-white/[0.04] transition-all">
                                <div>
                                    ...
                                    <p class="text-sm font-bold text-slate-400 italic leading-relaxed uppercase tracking-tight">"Protocol Authorization: refer to SOAP_LEDGER_<?php echo e(substr($e->id, 0, 8)); ?> for diagnostic rationale and treatment planning."</p>
                                </div>
                                <div class="flex gap-4 pt-4">
                                    <span class="px-4 py-1.5 bg-white/5 border border-white/10 rounded-xl text-[8px] font-black text-slate-500 uppercase tracking-widest italic">MATRIX_ID: <?php echo e(substr($e->id, 0, 12)); ?></span>
                                    <span class="px-4 py-1.5 bg-indigo-500/10 border border-indigo-500/20 rounded-xl text-[8px] font-black text-indigo-400 uppercase tracking-widest italic">STATE: <?php echo e(strtoupper($e->status)); ?></span>
                                </div>
                            </div>
                        </div>
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\opeshis\resources\views\clinical\dossier.blade.php ENDPATH**/ ?>