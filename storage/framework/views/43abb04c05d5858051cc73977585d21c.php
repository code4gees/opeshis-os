...
<div class="space-y-8 animate-fade-in pb-20">
 ...
 <div>
 <h1 class="text-4xl font-semibold text-white tracking-tighter uppercase leading-none"><?php echo e($patient->full_name); ?></h1>
 <div class="flex items-center gap-6 mt-4">
 <span class="text-[12px] font-semibold text-sage font-medium bg-sage/10 border border-indigo-500/20 px-4 py-1.5 rounded-xl /5"><?php echo e($patient->medical_id); ?></span>
 <span class="text-[12px] font-semibold text-slate-500 font-medium "><?php echo e($patient->gender); ?> · <?php echo e(\Carbon\Carbon::parse($patient->date_of_birth)->age); ?> Years</span>
 <span class="text-[12px] font-semibold text-rose-500 font-medium bg-rose-500/10 border border-rose-500/20 px-4 py-1.5 rounded-xl /5 ">BLOOD: <?php echo e($patient->blood_group); ?></span>
 </div>
 </div>
 </div>
 ...
 <a href="<?php echo e(route('emr.main', ['id' => $patient->id])); ?>" class="px-8 py-4 bg-sage text-white rounded-xl font-semibold text-[12px] font-medium /20 hover:bg-indigo-700 transition-all border border-indigo-500/50">
 ...
 <p class="text-sm font-semibold text-rose-400 uppercase leading-relaxed"><?php echo e($patient->allergies ?: 'NO_KNOWN_ALLERGIES_IDENTIFIED'); ?></p>
 </div>
 <div>
 <p class="text-[12px] font-semibold text-slate-500 font-medium mb-3 ">Genotype Identification</p>
 <p class="text-sm font-semibold text-white uppercase "><?php echo e($patient->genotype ?: 'NULL_SPEC'); ?></p>
 </div>
 </div>
 </div>
 ...
 <p class="text-sm font-semibold text-white mt-2 uppercase tracking-tighter">NODE: <?php echo e($currentAdmission->room_no ?? 'ICU_CORE_201'); ?></p>
 </div>
 <?php else: ?>
 ...
 <p class="text-[12px] font-bold text-slate-500 mt-1 font-medium ">Physician Node: <?php echo e($e->doctor_name); ?></p>
 </div>
 <span class="text-[12px] font-semibold text-slate-500 font-medium leading-none"><?php echo e(\Carbon\Carbon::parse($e->created_at)->format('d M Y')); ?></span>
 </div>
 ...
 <p class="text-sm font-bold text-slate-400 leading-relaxed uppercase tracking-tight">"Protocol Authorization: refer to SOAP_LEDGER_<?php echo e(substr($e->id, 0, 8)); ?> for diagnostic rationale and treatment planning."</p>
 </div>
 <div class="flex gap-4 pt-4">
 <span class="px-4 py-1.5 bg-[#2a2e38] border border-subtle rounded-xl text-[8px] font-semibold text-slate-500 font-medium ">MATRIX_ID: <?php echo e(substr($e->id, 0, 12)); ?></span>
 <span class="px-4 py-1.5 bg-sage/10 border border-indigo-500/20 rounded-xl text-[8px] font-semibold text-sage font-medium ">STATE: <?php echo e(strtoupper($e->status)); ?></span>
 </div>
 </div>
 </div>
