...
<?php $__env->startSection('title', 'Inpatient Ward Management — Opeshis OS'); ?>
        ...
        <div class="flex gap-6">
            <div class="flex items-center gap-8 px-10 py-4 glass-panel rounded-[2rem] border border-white/10 bg-white/[0.02]">
                <div class="text-center">
                    ...
                    <p class="text-3xl font-black text-white italic tracking-tighter leading-none"><?php echo e($occupancyRate); ?>%</p>
                </div>
                ...
                <div class="text-center">
                    ...
                    <p class="text-3xl font-black text-white italic tracking-tighter leading-none"><?php echo e($admissions->count()); ?></p>
                </div>
            </div>
            ...
            <div class="glass-panel p-10 rounded-[3rem] border border-white/10 shadow-2xl bg-white/[0.02] group hover:border-indigo-500/50 transition-all duration-500 relative overflow-hidden">
                <div class="flex justify-between items-start mb-8">
                    ...
                    <span class="px-3 py-1 bg-white/5 text-slate-500 border border-white/10 rounded-lg text-[8px] font-black uppercase tracking-widest italic">
                        <?php echo e($w->type ?: 'GENERAL_PROTOCOL'); ?>

                    </span>
                </div>
                <h4 class="text-xl font-black text-white uppercase mb-6 italic tracking-tighter"><?php echo e($w->name); ?></h4>
                <div class="space-y-6">
                    <div class="flex justify-between items-center">
                        <span class="text-[9px] font-black text-slate-500 uppercase tracking-widest italic">Available Beds</span>
                        <span class="text-lg font-black text-emerald-500 italic"><?php echo e($w->available); ?></span>
                    </div>
                    <?php $rate = $w->total > 0 ? ($w->occupied / $w->total) * 100 : 0; ?>
                    <div class="w-full bg-white/5 h-1.5 rounded-full overflow-hidden shadow-inner border border-white/5">
                        <div class="h-full bg-indigo-500 rounded-full shadow-lg shadow-indigo-500/20 transition-all duration-1000" style="width: <?php echo e($rate); ?>%"></div>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-[9px] font-black text-slate-500 uppercase tracking-widest italic">Occupancy Matrix</span>
                        <span class="text-xs font-black text-white italic tracking-widest"><?php echo e(round($rate)); ?>%</span>
                    </div>
                </div>
                        ...
                        <tr class="hover:bg-white/5 transition-all group">
                            <td class="px-10 py-6">
                                <div class="flex items-center gap-5">
                                    ...
                                    <div>
                                        <div class="font-black text-white text-base uppercase group-hover:text-indigo-400 transition-colors italic tracking-tight"><?php echo e($a->full_name); ?></div>
                                        <div class="text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em] mt-1 italic"><?php echo e($a->medical_id); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-6">
                                <div class="text-[11px] font-black text-white italic tracking-widest uppercase"><?php echo e($a->bed_number); ?></div>
                                <div class="text-[8px] font-bold text-slate-600 uppercase tracking-widest mt-1 italic">W_ID: <?php echo e(substr($a->ward_id, 0, 8)); ?></div>
                            </td>
                            <td class="px-6 py-6">
                                <div class="text-[11px] font-black text-slate-300 uppercase italic tracking-tight truncate max-w-xs leading-relaxed">"<?php echo e($a->admitting_diagnosis); ?>"</div>
                                <div class="text-[8px] font-bold text-slate-600 uppercase tracking-widest mt-1 italic">CONSULTANT: <?php echo e(strtoupper($a->consultant)); ?></div>
                            </td>
                            <td class="px-6 py-6 text-center">
                                <div class="text-[10px] font-black text-slate-400 uppercase italic tracking-widest"><?php echo e(\Carbon\Carbon::parse($a->admitted_at)->diffForHumans()); ?></div>
                            </td>
                            <td class="px-10 py-6 text-right">
                                <div class="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all">
                                    ...
                                    <button onclick="openDischargeModal('<?php echo e($a->id); ?>')" class="px-4 py-2 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded-xl text-[8px] font-black uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all italic">Discharge Protocol</button>
                                </div>
                            </td>
                        </tr>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
...
<div id="admitModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] hidden flex items-center justify-center p-8">
    <div class="glass-panel w-full max-w-xl rounded-[3rem] p-12 shadow-2xl border border-white/10">
        ...
        <form method="POST" action="<?php echo e(url('/clinical/ward/admit')); ?>" class="space-y-6">
            <?php echo csrf_field(); ?>
            <div>
            ...
            <div class="grid grid-cols-2 gap-8">
                <div>
                    ...
                    <select name="ward_id" id="wardSelect" onchange="updateBeds()" class="w-full bg-white/5 border border-white/10 rounded-2xl px-8 py-5 text-sm font-black text-white outline-none focus:border-indigo-500/50 transition-all">
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $wards; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $w): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                            <option value="<?php echo e($w->id); ?>" class="bg-slate-900 italic uppercase"><?php echo e($w->name); ?> (<?php echo e($w->available); ?> AVAIL)</option>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                    </select>
                </div>
                <div>
...
<script>
    function updateBeds() {
        ...
        fetch(`<?php echo e(url('/clinical/ward/available-beds')); ?>?ward_id=${wardId}`)
            .then(res => res.json())
            .then(data => {
