...
<div class="max-w-7xl mx-auto space-y-8 animate-fade-in pb-20">
 ...
 <a href="{{ route('appointments.index', ['date' => \Carbon\Carbon::parse($date)->subDay()->toDateString()]) }}" class="p-3 text-slate-400 hover:text-sage transition-colors">
 ...
 <a href="{{ route('appointments.index', ['date' => \Carbon\Carbon::parse($date)->addDay()->toDateString()]) }}" class="p-3 text-slate-400 hover:text-sage transition-colors">
 <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"/></svg>
 </a>
 </div>
 <div class="relative">
 <input type="date" value="{{ $date }}" onchange="location.href='?date='+this.value" class="bg-card border border-subtle rounded-lg px-6 py-2.5 text-xs font-bold font-medium text-slate-200 outline-none focus:ring-2 focus:ring-blue-600 transition-all cursor-pointer">
 </div>
 </div>
 ...
 <h3 class="text-[12px] font-bold text-amber-500 font-medium mb-8">Pending Portal Requests ({{ $requests->count() }})</h3>
 ...
 <div>
 <h4 class="text-xs font-bold text-white uppercase tracking-tight group-hover:text-amber-400 transition-colors">{{ $req->full_name }}</h4>
 <p class="text-[12px] text-slate-500 font-bold uppercase mt-1">{{ $req->medical_id }}</p>
 </div>
 <span class="px-2 py-0.5 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded text-[12px] font-bold uppercase">{{ $req->urgency }}</span>
 </div>
 <p class="text-[12px] text-slate-400 mb-6 border-l-2 border-amber-500/20 pl-4">"{{ $req->reason }}"</p>
 ...
 <span class="text-[12px] font-bold text-sage font-medium">Confirmed Today: {{ $appointments->where('status', 'confirmed')->count() }}</span>
 </div>
 ...
 <td class="px-8 py-6">
 <div class="font-bold text-white uppercase text-xs">{{ $a->full_name }}</div>
 <div class="text-[12px] text-slate-500 font-bold font-medium mt-0.5">{{ $a->appointment_number }}</div>
 </td>
 <td class="px-8 py-6">
 <div class="text-xs font-bold text-slate-300 uppercase tracking-tight">{{ $a->doctor_name ?? 'Not Assigned' }}</div>
 <div class="text-[12px] text-slate-500 font-bold font-medium mt-1">{{ $a->dept_name ?? 'General Unit' }}</div>
 </td>
 ...
 @endphp
 <span class="px-2 py-0.5 rounded text-[12px] font-bold uppercase border {{ $cls }}">
 {{ $a->status }}
 </span>
 </td>
...
<div id="bookModal" class="fixed inset-0 bg-card/80 z-[100] hidden flex items-center justify-center p-6">
 ...
 <input type="date" name="date" required value="{{ $date }}" class="w-full bg-card border border-slate-700 rounded-lg px-4 py-3 text-sm font-bold text-slate-200 outline-none focus:ring-2 focus:ring-blue-600 transition-all">
 </div>
 <div>
