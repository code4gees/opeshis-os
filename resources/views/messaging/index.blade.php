    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        <div class="glass-panel rounded-[2.5rem] p-10 border border-white/10 shadow-xl bg-gradient-to-br from-indigo-500/5 to-transparent flex flex-col justify-center">
            ...
            <div class="flex items-end gap-3">
                <h2 class="text-5xl font-black text-white italic tracking-tighter">{{ $stats->pending ?? 0 }}</h2>
        ...
        <div class="glass-panel rounded-[2.5rem] p-10 border border-white/10 shadow-xl bg-gradient-to-br from-emerald-500/5 to-transparent flex flex-col justify-center">
            ...
            <div class="flex items-end gap-3">
                <h2 class="text-5xl font-black text-emerald-500 italic tracking-tighter">{{ $stats->sent_today ?? 0 }}</h2>
        ...
        <div class="glass-panel rounded-[2.5rem] p-10 border border-white/10 shadow-xl bg-gradient-to-br from-rose-500/5 to-transparent flex flex-col justify-center relative overflow-hidden">
            ...
            <div class="flex items-end gap-3">
                <h2 class="text-5xl font-black text-rose-500 italic tracking-tighter">{{ $stats->failed_today ?? 0 }}</h2>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="hover:bg-white/5 transition-all group">
                        ...
                        <td class="px-6 py-6">
                            <div class="font-black text-white text-sm uppercase group-hover:text-indigo-400 transition-colors">{{ $m->recipient_name ?: 'ANONYMOUS_RECIPIENT' }}</div>
                            <div class="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1 italic">{{ $m->recipient_phone }}</div>
                        </td>
                        <td class="px-6 py-6">
                            <span class="px-3 py-1 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-lg text-[8px] font-black uppercase tracking-widest">{{ $m->channel }}</span>
                        </td>
                        <td class="px-6 py-6">
                            <div class="text-[11px] text-slate-400 max-w-sm truncate italic group-hover:whitespace-normal transition-all leading-relaxed uppercase tracking-tight">"{{ $m->resolved_message }}"</div>
                            <div class="text-[7px] font-black text-slate-600 uppercase mt-2 tracking-widest">RELAY_SOURCE: {{ $m->provider ?? 'INSTITUTIONAL_CORE' }}</div>
                        </td>
                        <td class="px-10 py-6 text-right">
                            ...
                            @endphp
                            <span class="px-4 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest border {{ $statusCls }} shadow-lg shadow-white/5">
                                {{ $m->status }}
                            </span>
                        </td>
                    </tr>
