<aside class="w-72 glass-panel border-r border-white/5 flex flex-col h-screen sticky top-0" style="width:288px;min-width:288px">
    ...
    <nav class="flex-1 overflow-y-auto px-4 pb-4 space-y-0.5 no-scrollbar">
        ...
        {!! $nav('/dashboard','Hospital Dashboard','dashboard') !!}
        
        @if($hasPerm('module_patients') || $hasPerm('core_admin'))
            {!! $nav('/patients','Patient Registry','patients') !!}
        @endif
        
        @if($hasPerm('module_appointments') || $hasPerm('core_admin'))
            {!! $nav('/appointments','Appointments','appointments') !!}
        @endif
        
        @if($hasPerm('module_messaging') || $hasPerm('core_admin'))
            {!! $nav('/messaging','Messaging & Alerts','messaging') !!}
        @endif
        ...
        @if($hasPerm('module_clinical') || $hasPerm('module_vitals') || $hasPerm('module_emergency') || $hasPerm('module_maternal') || $hasPerm('module_paeds') || $hasPerm('module_chronic') || $hasPerm('module_telemedicine') || $hasPerm('core_admin'))
            ...
            @if($hasPerm('module_clinical') || $hasPerm('core_admin'))
                {!! $nav('/emr','Consultations (EMR)','emr') !!}
            @endif

            @if($hasPerm('module_vitals') || $hasPerm('core_admin'))
                {!! $nav('/triage','Triage & Vitals','triage') !!}
            @endif

            @if($hasPerm('module_emergency') || $hasPerm('core_admin'))
                {!! $nav('/emergency','Emergency & Trauma','emergency') !!}
            @endif

            @if($hasPerm('module_maternal') || $hasPerm('core_admin'))
                {!! $nav('/maternal','Maternal Health (ANC)','maternal') !!}
            @endif

            @if($hasPerm('module_paeds') || $hasPerm('core_admin'))
                {!! $nav('/paeds','Pediatrics','paeds') !!}
            @endif

            @if($hasPerm('module_chronic') || $hasPerm('core_admin'))
                {!! $nav('/ncd','Chronic Care (NCD)','ncd') !!}
            @endif

            @if($hasPerm('module_telemedicine') || $hasPerm('core_admin'))
                {!! $nav('/telemedicine','Telemedicine Hub','telemedicine') !!}
            @endif
        @endif
        ...
        @if($hasPerm('module_pharmacy') || $hasPerm('module_lab') || $hasPerm('module_radiology') || $hasPerm('core_admin'))
            ...
            @if($hasPerm('module_pharmacy') || $hasPerm('core_admin'))
                {!! $nav('/pharmacy','Pharmacy & Dispensary','pharmacy') !!}
            @endif

            @if($hasPerm('module_lab') || $hasPerm('core_admin'))
                {!! $nav('/lab','Laboratory','lab') !!}
            @endif

            @if($hasPerm('module_radiology') || $hasPerm('core_admin'))
                {!! $nav('/radiology','Radiology & Imaging','radiology') !!}
            @endif
        @endif
        ...
        @if($hasPerm('module_warehouse') || $hasPerm('module_assets') || $hasPerm('module_billing') || $hasPerm('core_admin'))
            ...
            @if($hasPerm('module_warehouse') || $hasPerm('core_admin'))
                {!! $nav('/warehouse','Warehouse & Stock','warehouse') !!}
            @endif

            @if($hasPerm('module_assets') || $hasPerm('core_admin'))
                {!! $nav('/assets','Asset Register','assets') !!}
            @endif

            @if($hasPerm('module_billing') || $hasPerm('core_admin'))
                {!! $nav('/billing','Billing & Finance','billing') !!}
            @endif
        @endif
        ...
        @if($hasPerm('core_admin') || $hasPerm('module_tech'))
            ...
            @if($hasPerm('core_admin') || $hasPerm('module_tech'))
                {!! $nav('/analytics','Data Analytics (BI)','analytics') !!}
            @endif
            
            @if($hasPerm('core_admin'))
                {!! $nav('/admin','Hospital Admin','admin') !!}
                {!! $nav('/settings','System Settings','settings') !!}
            @endif
        @endif
