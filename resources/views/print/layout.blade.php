...
<body>
 ...
 <div class="meta">
 <div><strong>REF:</strong> #{{ strtoupper(substr($type, 0, 3)) }}-{{ substr($data->id, 0, 8) }}</div>
 ...
 <span>{{ $data->medical_id }}</span>
 </div>
 <div class="group">
 <label>Bio Stats</label>
 <span>{{ $data->gender }} / {{ \Carbon\Carbon::parse($data->dob)->age }} YRS</span>
 </div>
 </div>
 ...
 @if(in_array($key, ['id', 'patient_id', 'full_name', 'medical_id', 'gender', 'dob', 'created_at', 'updated_at']) || empty($value)) @continue @endif
 <tr>
 <td><span style="text-transform: uppercase; font-size: 9px; color: #94a3b8;">{{ str_replace('_', ' ', $key) }}</span></td>
 <td>{{ $value }}</td>
 </tr>
 @endforeach
 </tbody>
 ...
 <div>Institutional UUID: {{ strtoupper(Str::random(12)) }}</div>
 </div>
 </div>
</body>
</html>
