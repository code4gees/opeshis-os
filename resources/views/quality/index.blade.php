...
<div class="space-y-8 animate-fade-in pb-20">
 ...
 <span class="px-3 py-1 bg-[#2a2e38] text-slate-400 rounded-lg text-[8px] font-semibold font-medium border border-subtle ">Ledger Sync: {{ $incidents->count() }} Entries</span>
 </div>
 ...
 @endphp
 <span class="px-4 py-1.5 rounded-xl text-[8px] font-semibold font-medium border {{ $sevCls }} w-max">
 {{ $i->severity_level }} SEVERITY
 </span>
 <h3 class="text-2xl font-semibold text-white uppercase tracking-tighter group-hover:text-rose-400 transition-colors">{{ $i->incident_type }}</h3>
 <p class="text-[12px] font-semibold text-slate-500 font-medium ">{{ $i->location ?? 'UNSPECIFIED_INSTITUTIONAL_LOCATION' }} · {{ \Carbon\Carbon::parse($i->incident_date)->format('M j, Y · H:i') }}</p>
 </div>
 <div class="text-right">
 <span class="block text-[8px] font-semibold text-slate-600 font-medium mb-2 ">Tracking Vector</span>
 <span class="px-3 py-1 bg-[#2a2e38] text-sage rounded-lg text-[12px] font-semibold font-medium border border-subtle">{{ $i->status }}</span>
 </div>
 </div>
 
 <div class="p-8 bg-[#2a2e38] rounded-[2.5rem] border border-subtle mb-10">
 <p class="text-slate-300 text-[13px] font-bold leading-relaxed uppercase tracking-tight">"{{ $i->description }}"</p>
 </div>
 ...
 <span class="text-[12px] font-semibold text-rose-500 font-medium ">{{ $c->incident_type }}</span>
 <span class="px-2 py-1 bg-white/10 text-white rounded text-[7px] font-semibold font-medium border border-subtle">{{ $c->status }}</span>
 </div>
 <div class="p-6 bg-[#2a2e38] rounded-[2rem] border border-subtle mb-8 relative z-10">
 <p class="text-[12px] font-bold text-slate-300 leading-relaxed uppercase tracking-tight ">{{ $c->corrective_action }}</p>
 </div>
