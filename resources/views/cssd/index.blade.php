...
<div class="space-y-8 animate-fade-in">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
        <div class="glass-panel rounded-[2.5rem] p-10 border border-white/10 shadow-xl bg-gradient-to-br from-indigo-500/5 to-transparent flex flex-col justify-center">
            ...
            <h3 class="text-4xl font-black text-white italic tracking-tighter">{{ $loadStats->total ?? 0 }}</h3>
        </div>
        <div class="glass-panel rounded-[2.5rem] p-10 border border-white/10 shadow-xl bg-gradient-to-br from-emerald-500/5 to-transparent flex flex-col justify-center">
            ...
            <h3 class="text-4xl font-black text-emerald-500 italic tracking-tighter">{{ $loadStats->passed ?? 0 }}</h3>
        </div>
        <div class="glass-panel rounded-[2.5rem] p-10 border border-white/10 shadow-xl bg-gradient-to-br from-rose-500/5 to-transparent flex flex-col justify-center">
            ...
            <h3 class="text-4xl font-black text-rose-500 italic tracking-tighter">{{ $loadStats->failed ?? 0 }}</h3>
        </div>
        <div class="glass-panel rounded-[2.5rem] p-10 border border-white/10 shadow-xl bg-gradient-to-br from-amber-500/5 to-transparent flex flex-col justify-center relative overflow-hidden">
            ...
            <h3 class="text-4xl font-black text-white italic tracking-tighter">{{ $expiringCount }}</h3>
            @if($expiringCount > 0)
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-white/5 transition-all group">
                            <td class="px-10 py-6">
                                <div class="font-black text-indigo-400 text-sm uppercase group-hover:text-indigo-300 transition-colors">{{ $l->load_number }}</div>
                                <div class="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-0.5 italic">EXPIRY_SYNC: {{ \Carbon\Carbon::parse($l->expiry_date)->format('d M Y') }}</div>
                            </td>
                            <td class="px-6 py-6">
                                <div class="text-[10px] font-black text-white uppercase tracking-widest italic">{{ $l->sterilizer_name }}</div>
                            </td>
                            <td class="px-6 py-6">
                                <div class="text-[9px] font-black text-slate-500 uppercase tracking-widest">{{ $l->sterilization_method }}</div>
                            </td>
            ...
            <div class="glass-panel rounded-[3rem] p-10 border border-white/10 shadow-xl relative overflow-hidden bg-white/[0.02]">
                ...
                <div class="space-y-4 relative z-10">
                    @foreach($sterilizers as $st)
                        ...
                        <div class="p-6 rounded-3xl border {{ $overdue ? 'border-rose-500/30 bg-rose-500/5 shadow-lg shadow-rose-500/5' : 'border-white/5 bg-white/5' }} transition-all group hover:border-indigo-500/30">
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                    <h3 class="text-[11px] font-black text-white uppercase tracking-tight group-hover:text-indigo-400 transition-colors">{{ $st->sterilizer_name }}</h3>
                                    <p class="text-[8px] font-black text-slate-500 uppercase mt-1 tracking-widest">{{ $st->sterilizer_type }}</p>
                                </div>
                                @if($overdue)
