...
<div class="space-y-8 animate-fade-in">
 ...
 <p class="text-xl font-semibold text-emerald-500 leading-none">{{ $courses->count() }}</p>
 </div>
 <div class="w-px h-8 bg-white/10"></div>
 <div class="text-center">
 <p class="text-[8px] font-semibold text-sage font-medium mb-1">Scheduled Sessions</p>
 <p class="text-xl font-semibold text-sage leading-none">{{ $sessions->count() }}</p>
 </div>
 </div>
 ...
 <div>
 <div class="text-sm font-semibold text-white uppercase group-hover:text-emerald-400 transition-colors">{{ $c->title }}</div>
 <div class="text-[12px] font-bold text-slate-500 font-medium mt-0.5">Target: {{ $c->target_role }}</div>
 </div>
 </div>
 @if($c->mandatory)
 ...
 <p class="text-lg font-semibold text-white leading-none">{{ \Carbon\Carbon::parse($s->session_date)->format('d') }}</p>
 <p class="text-[8px] font-semibold text-sage font-medium mt-1">{{ \Carbon\Carbon::parse($s->session_date)->format('M') }}</p>
 </div>
 <div class="w-px h-10 bg-white/10"></div>
 <div>
 <div class="text-sm font-semibold text-white uppercase">{{ $s->course_title }}</div>
 <div class="text-[12px] font-bold text-slate-500 font-medium mt-0.5">{{ $s->venue }} · {{ \Carbon\Carbon::parse($s->session_date)->format('H:i') }}</div>
 </div>
 </div>
 @endforeach
 @if($sessions->isEmpty())
