    ...
    <div class="w-full max-w-md p-10 login-card rounded-[3rem] shadow-2xl relative z-10">
        ...
        <form id="loginForm" action="{{ route('login.post') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" required placeholder="name@opesware.com" class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white outline-none focus:border-indigo-500 transition">
            </div>
