...
<div class="space-y-8 animate-fade-in">
 ...
 <span class="px-3 py-1 bg-[#2a2e38] text-slate-400 rounded-lg text-[8px] font-semibold font-medium border border-subtle ">Daily Sync: {{ now()->format('d M Y') }}</span>
 </div>
 ...
 <td class="px-10 py-6">
 <div class="font-semibold text-white text-sm uppercase group-hover:text-emerald-400 transition-colors">{{ $p->full_name }}</div>
 <div class="text-[12px] font-bold text-slate-500 font-medium mt-0.5">{{ $p->medical_id }}</div>
 </td>
 <td class="px-6 py-6">
 <span class="px-3 py-1 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-lg text-[8px] font-semibold font-medium">{{ $p->meal_type }}</span>
 </td>
 <td class="px-6 py-6">
 <div class="text-[12px] text-slate-400 max-w-xs truncate group-hover:whitespace-normal group-hover:overflow-visible transition-all">"{{ $p->restrictions ?: 'No documented clinical restrictions' }}"</div>
 </td>
 ...
 @else
 <button onclick="logMealDelivery('{{ $p->id }}', '{{ $meal }}')" class="w-10 h-10 bg-[#2a2e38] text-slate-500 rounded-xl flex items-center justify-center mx-auto hover:bg-sage/20 hover:text-sage transition-all border border-subtle hover:border-indigo-500/30">
