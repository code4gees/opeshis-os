...
<div class="space-y-2">
    ...
    <div class="relative group">
        ...
        <select 
            name="{{ $name }}" 
            id="{{ $name }}" 
            {{ $attributes->merge(['class' => ($icon ? 'pl-11' : 'px-4') . ' w-full bg-slate-900/50 border border-white/5 rounded-xl py-3 text-xs font-bold text-white outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/10 transition-all backdrop-blur-md appearance-none']) }}
        >
            {{ $slot }}
        </select>
