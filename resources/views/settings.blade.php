...
<div class="max-w-7xl mx-auto space-y-8 animate-fade-in pb-20">
 ...
 <input type="text" name="settings[hospital_name]" value="{{ $settings['hospital_name'] ?? 'Opeshis General Hospital' }}" required class="w-full bg-card border border-slate-700 rounded-lg px-4 py-3 text-sm text-slate-200 outline-none focus:ring-2 focus:ring-blue-600">
 </div>
 <div>
 <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Primary Email</label>
 <input type="email" name="settings[hospital_email]" value="{{ $settings['hospital_email'] ?? 'contact@opeshis.com' }}" required class="w-full bg-card border border-slate-700 rounded-lg px-4 py-3 text-sm text-slate-200 outline-none focus:ring-2 focus:ring-blue-600">
 </div>
 <div>
 <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Phone Number</label>
 <input type="text" name="settings[hospital_phone]" value="{{ $settings['hospital_phone'] ?? '+237 000 000 000' }}" required class="w-full bg-card border border-slate-700 rounded-lg px-4 py-3 text-sm text-slate-200 outline-none focus:ring-2 focus:ring-blue-600">
 </div>
 </div>
 </div>
