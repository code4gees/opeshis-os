...
@section('title', 'Chronic Care (NCD) - Opeshis OS')
...
<div class="max-w-7xl mx-auto space-y-8 animate-fade-in pb-20">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="bg-slate-800 p-6 rounded-xl border border-slate-700/60 shadow-sm relative overflow-hidden">
            ...
            <h3 class="text-3xl font-bold text-slate-100 tracking-tight">{{ $registry->count() }} <span class="text-xs font-medium text-slate-500 ml-1 uppercase">Patients</span></h3>
                        ...
                        @forelse ($registry as $r)
                            <tr class="hover:bg-slate-700/30 transition-colors">
                                <td class="px-8 py-6">
                                    ...
                                    <div class="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">{{ $r->medical_id }}</div>
                                </td>
                                <td class="px-6 py-6">
                                    <span class="px-2 py-0.5 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded text-[9px] font-bold uppercase tracking-wider">{{ $r->condition_type }}</span>
                                </td>
                                <td class="px-6 py-6 text-[11px] font-bold text-slate-400 uppercase">
                                    {{ $r->last_seen ? \Carbon\Carbon::parse($r->last_seen)->format('d M, Y') : 'Pending Intake' }}
                                </td>
                                ...
                                <td class="px-8 py-6 text-right">
                                    <button onclick="viewTrends('{{ $r->patient_id }}', '{{ addslashes($r->full_name) }}')" class="inline-flex items-center gap-2 text-[10px] font-bold text-blue-500 uppercase hover:text-blue-400 transition-colors">
                                        View Trends
...
<div id="enrollModal" class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[100] hidden flex items-center justify-center p-6">
    <div class="bg-slate-800 w-full max-w-xl rounded-xl p-8 shadow-2xl border border-slate-700/60">
        ...
        <form method="POST" action="{{ route('ncd.enroll') }}" class="space-y-6">
            @csrf
            <div>
