...
<div class="max-w-7xl mx-auto space-y-8 animate-fade-in pb-20">
 ...
 <h3 class="text-3xl font-bold text-white tracking-tight">{{ $registry->count() }} <span class="text-xs font-medium text-slate-500 ml-1 uppercase">Patients</span></h3>
 ...
 <div class="text-[12px] text-slate-500 font-bold font-medium mt-0.5">{{ $r->medical_id }}</div>
 </td>
 <td class="px-6 py-6">
 <span class="px-2 py-0.5 bg-sage/10 text-sage border border-blue-500/20 rounded text-[12px] font-bold font-medium">{{ $r->condition_type }}</span>
 </td>
 ...
 <button onclick="viewTrends('{{ $r->patient_id }}', '{{ addslashes($r->full_name) }}')" class="inline-flex items-center gap-2 text-[12px] font-bold text-sage uppercase hover:text-sage transition-colors">
 View Trends
