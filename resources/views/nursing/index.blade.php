...
<div class="space-y-8 animate-fade-in pb-20">
 ...
 <span class="px-3 py-1 bg-[#2a2e38] text-slate-500 rounded-lg text-[8px] font-semibold font-medium border border-subtle ">{{ $a->ward_name }}</span>
 <div class="text-[12px] font-semibold text-sage mt-2 uppercase tracking-tighter">Bed: {{ $a->bed_number }}</div>
 </div>
 </div>

 <div class="mb-10 relative z-10">
 <h4 class="text-xl font-semibold text-white uppercase tracking-tighter group-hover:text-sage transition-colors">{{ $a->full_name }}</h4>
 <p class="text-[12px] font-semibold text-slate-500 font-medium mt-1 ">{{ $a->medical_id }} · {{ strtoupper($a->gender) }}</p>
 </div>
 ...
 <div class="text-sm font-semibold {{ ($a->last_vitals->temp ?? 0) > 37.5 ? 'text-rose-500' : 'text-white' }}">{{ $a->last_vitals->temp ?? '--' }}°C</div>
 </div>
 <div class="bg-[#2a2e38] rounded-3xl p-5 border border-subtle text-center group-hover:border-subtle transition-all">
 <label class="text-[8px] font-semibold text-slate-600 uppercase block mb-2 tracking-wider">BP_SYS_DIA</label>
 <div class="text-sm font-semibold text-white ">{{ $a->last_vitals->bp_sys ?? '--' }}/{{ $a->last_vitals->bp_dia ?? '--' }}</div>
 </div>
 <div class="bg-[#2a2e38] rounded-3xl p-5 border border-subtle text-center group-hover:border-subtle transition-all">
 <label class="text-[8px] font-semibold text-slate-600 uppercase block mb-2 tracking-wider">SPO2_OXY</label>
 <div class="text-sm font-semibold text-emerald-500 ">{{ $a->last_vitals->spo2 ?? '--' }}%</div>
 </div>
 <div class="bg-[#2a2e38] rounded-3xl p-5 border border-subtle text-center group-hover:border-subtle transition-all">
 <label class="text-[8px] font-semibold text-slate-600 uppercase block mb-2 tracking-wider">RESP_RATE</label>
 <div class="text-sm font-semibold text-white ">{{ $a->last_vitals->respiratory_rate ?? '--' }}</div>
 </div>
 </div>
 ...
 @endphp
 <div class="mb-10 p-6 {{ $riskColor }} rounded-[2rem] border flex justify-between items-center relative z-10">
 <div class="flex flex-col">
 <span class="text-[8px] font-semibold font-medium mb-1 ">NEWS2 Clinical Risk</span>
 <span class="text-xs font-semibold uppercase">{{ $a->last_vitals->risk_level }} PROTOCOL</span>
 </div>
 <span class="text-2xl font-semibold tracking-tighter">{{ $a->last_vitals->news2_score }}</span>
 </div>
 @endif

 <div class="flex gap-4 relative z-10">
 <button onclick="openRoundModal('{{ $a->id }}', '{{ addslashes($a->full_name) }}')" class="flex-1 py-4 bg-sage text-white rounded-2xl text-[12px] font-semibold font-medium /20 hover:bg-indigo-700 transition-all border border-indigo-500/50">Authorize Round</button>
 <a href="{{ route('patients.show', $a->patient_id) }}" class="flex-1 py-4 bg-[#2a2e38] text-slate-400 border border-subtle rounded-2xl text-[12px] font-semibold font-medium text-center hover:bg-white/10 transition-all ">Longitudinal Profile</a>
 </div>
