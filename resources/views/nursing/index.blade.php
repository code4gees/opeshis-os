...
@section('title', 'Nursing Strategic Command — Opeshis OS')
    ...
    <header class="flex justify-between items-center mb-10 pb-8 border-b border-white/10">
        ...
        <div class="flex gap-4">
            <span class="px-6 py-3 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-xl text-[10px] font-black uppercase tracking-widest italic shadow-lg shadow-indigo-500/5">
                Live Census: {{ $admissions->count() }} Patients
            </span>
        </div>
    </header>
            ...
            <div class="glass-panel rounded-[3rem] p-10 border border-white/10 shadow-xl hover:border-indigo-500/30 transition-all group bg-white/[0.02] relative overflow-hidden">
                <div class="flex justify-between items-start mb-8 relative z-10">
                    ...
                    <div class="text-right">
                        <span class="px-3 py-1 bg-white/5 text-slate-500 rounded-lg text-[8px] font-black uppercase tracking-widest border border-white/10 italic">{{ $a->ward_name }}</span>
                        <div class="text-[11px] font-black text-indigo-400 mt-2 uppercase tracking-tighter">Bed: {{ $a->bed_number }}</div>
                    </div>
                </div>

                <div class="mb-10 relative z-10">
                    <h4 class="text-xl font-black text-white uppercase tracking-tighter group-hover:text-indigo-400 transition-colors">{{ $a->full_name }}</h4>
                    <p class="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mt-1 italic">{{ $a->medical_id }} · {{ strtoupper($a->gender) }}</p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-8 relative z-10">
                    <div class="bg-white/5 rounded-3xl p-5 border border-white/5 text-center group-hover:border-white/10 transition-all">
                        ...
                        <div class="text-sm font-black italic {{ ($a->last_vitals->temp ?? 0) > 37.5 ? 'text-rose-500' : 'text-white' }}">{{ $a->last_vitals->temp ?? '--' }}°C</div>
                    </div>
                    <div class="bg-white/5 rounded-3xl p-5 border border-white/5 text-center group-hover:border-white/10 transition-all">
                        <label class="text-[8px] font-black text-slate-600 uppercase block mb-2 tracking-widest">BP_SYS_DIA</label>
                        <div class="text-sm font-black text-white italic">{{ $a->last_vitals->bp_sys ?? '--' }}/{{ $a->last_vitals->bp_dia ?? '--' }}</div>
                    </div>
                    <div class="bg-white/5 rounded-3xl p-5 border border-white/5 text-center group-hover:border-white/10 transition-all">
                        <label class="text-[8px] font-black text-slate-600 uppercase block mb-2 tracking-widest">SPO2_OXY</label>
                        <div class="text-sm font-black text-emerald-500 italic">{{ $a->last_vitals->spo2 ?? '--' }}%</div>
                    </div>
                    <div class="bg-white/5 rounded-3xl p-5 border border-white/5 text-center group-hover:border-white/10 transition-all">
                        <label class="text-[8px] font-black text-slate-600 uppercase block mb-2 tracking-widest">RESP_RATE</label>
                        <div class="text-sm font-black text-white italic">{{ $a->last_vitals->respiratory_rate ?? '--' }}</div>
                    </div>
                </div>

                @if(isset($a->last_vitals->news2_score))
                    @php 
                    ...
                    @endphp
                    <div class="mb-10 p-6 {{ $riskColor }} rounded-[2rem] border flex justify-between items-center shadow-lg relative z-10">
                        <div class="flex flex-col">
                            <span class="text-[8px] font-black uppercase tracking-[0.2em] mb-1 italic">NEWS2 Clinical Risk</span>
                            <span class="text-xs font-black uppercase">{{ $a->last_vitals->risk_level }} PROTOCOL</span>
                        </div>
                        <span class="text-2xl font-black italic tracking-tighter">{{ $a->last_vitals->news2_score }}</span>
                    </div>
                @endif

                <div class="flex gap-4 relative z-10">
                    <button onclick="openRoundModal('{{ $a->id }}', '{{ addslashes($a->full_name) }}')" class="flex-1 py-4 bg-indigo-600 text-white rounded-2xl text-[9px] font-black uppercase tracking-widest shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 transition-all border border-indigo-500/50">Authorize Round</button>
                    <a href="{{ route('patients.show', $a->patient_id) }}" class="flex-1 py-4 bg-white/5 text-slate-400 border border-white/10 rounded-2xl text-[9px] font-black uppercase tracking-widest text-center hover:bg-white/10 transition-all italic">Longitudinal Profile</a>
                </div>
...
<div id="roundModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] hidden flex items-center justify-center p-8">
    <div class="glass-panel w-full max-w-2xl rounded-[3rem] p-12 shadow-2xl border border-white/10">
        ...
        <form method="POST" action="{{ route('nursing.save') }}" class="space-y-6">
            @csrf
