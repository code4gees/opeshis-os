...
<div class="space-y-8 pb-20 animate-fade-in">
    ...
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10 pb-8 border-b border-white/5">
        <div>
            <h1 class="text-3xl font-black text-white tracking-tight uppercase">Psychiatry <span class="text-indigo-500">Command</span></h1>
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Institutional Mental Health · Risk Stratification · Behavioral Health Intelligence</p>
        </div>
            ...
            @forelse($patients as $p)
                <tr class="group hover:bg-indigo-500/[0.02] transition-colors border-b border-white/5 last:border-0">
                    ...
                    <td class="px-6 py-4">
                        ...
                        <div class="text-[8px] font-black text-slate-500 uppercase mt-1 tracking-widest">{{ strtoupper($p->status ?: 'OUTPATIENT') }}</div>
                    </td>
                    <td class="px-6 py-4">
                        @php
                        ...
                        <div class="text-[8px] font-black text-slate-500 uppercase mt-1 tracking-widest">{{ strtoupper($p->risk_level ?: 'NOT_ASSESSED') }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-[10px] font-black text-slate-400 uppercase italic tracking-widest">{{ $p->latest_mse ? \Carbon\Carbon::parse($p->latest_mse->created_at)->diffForHumans() : 'SIGNAL_MISSING' }}</div>
                        <div class="text-[8px] font-bold text-slate-600 uppercase tracking-widest mt-1">Last Evaluation</div>
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-6 py-24 text-center">
                        ...
                        <p class="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] italic">No active behavioral health principals identified in the matrix.</p>
                    </td>
                </tr>
            @endforelse
        </x-data-table>
...
<x-cc-modal id="regModal" title="Authorize Behavioral Enrollment" icon="fa-user-plus">
    <form method="POST" action="{{ url('/clinical/psych/register') }}" class="space-y-6">
        ...
        <div>
            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 italic">Referral Origin Source</label>
            <input name="source" required placeholder="REFERRAL_SOURCE_NODE" class="w-full bg-slate-900/50 border border-white/10 rounded-2xl px-6 py-4 text-sm font-black text-white outline-none focus:border-indigo-500/50 transition-all uppercase">
        </div>
        <div>
            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 italic">Clinical Presentation Disclosure</label>
            <textarea name="complaint" required rows="3" placeholder="PRESENTING_PICTURE_DISCLOSURE..." class="w-full bg-slate-900/50 border border-white/10 rounded-2xl px-6 py-4 text-sm font-black text-white outline-none focus:border-indigo-500/50 transition-all uppercase"></textarea>
        </div>
