...
<div class="space-y-8 pb-20">
 ...
 <div class="ml-4">
 <div class="text-sm font-bold text-slate-200 uppercase tracking-tight group-hover:text-amber-400 transition-colors ">{{ $w->patient->full_name }}</div>
 <div class="text-[12px] font-semibold text-slate-500 font-medium">{{ $w->patient->medical_id }}</div>
 </div>
 </div>
 </td>
 <td class="px-5 py-4">
 <div class="text-xs font-semibold text-slate-200 uppercase tracking-tight">{{ $w->wound_type }}</div>
 <div class="text-[8px] font-semibold text-slate-500 font-medium mt-1">Stage: {{ $w->stage }} · Size: {{ $w->size_cm }}cm</div>
 </td>
 <td class="whitespace-nowrap px-5 py-4">
 ...
 @if($latest)
 <div class="text-[12px] font-semibold text-slate-300 uppercase line-clamp-1">"{{ $latest->wound_bed }}"</div>
 <div class="text-[8px] font-bold text-slate-500 font-medium mt-1">Synced: {{ \Carbon\Carbon::parse($latest->created_at)->diffForHumans() }}</div>
 @else
...
<x-cc-modal id="dressingModal" title="Commit Dressing Assessment Intelligence" icon="fa-pen-to-square">
 ...
 <div class="pt-4">
 <x-cc-button type="submit" color="amber" class="w-full font-medium">Commit Dressing Intelligence</x-cc-button>
 </div>
 </form>
</x-cc-modal>
