...
<div class="space-y-8 pb-20 animate-fade-in">
 ...
 <td class="px-6 py-4">
 <div class="font-semibold text-white uppercase text-xs group-hover:text-sky-400 transition-colors ">{{ $s->patient->full_name }}</div>
 <div class="text-[12px] text-slate-500 mt-1 uppercase">{{ $s->patient->medical_id }}</div>
 </td>
 <td class="px-6 py-4">
 <p class="text-[12px] font-semibold text-slate-400 leading-relaxed uppercase tracking-tight line-clamp-2">"{{ $s->indication }}"</p>
 </td>
 <td class="px-6 py-4">
 @if($s->measurement)
 <div class="flex items-center gap-4">
 <x-status-badge status="completed" />
 <div class="text-[12px] font-semibold text-white leading-none">{{ $s->measurement->fev1_fvc_ratio }}% <span class="text-[8px] text-slate-500 ml-1">FEV1/FVC</span></div>
 </div>
 @else
