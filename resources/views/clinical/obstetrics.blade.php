...
<div class="space-y-8 pb-20 animate-fade-in">
 ...
 <td class="px-6 py-4">
 <div class="font-semibold text-white uppercase text-xs group-hover:text-rose-400 transition-colors ">{{ $c->patient->full_name }}</div>
 <div class="text-[12px] text-slate-500 mt-1 uppercase">{{ $c->patient->medical_id }}</div>
 </td>
 <td class="px-6 py-4 text-center">
 <div class="text-[12px] font-semibold text-white bg-card/50 py-1 px-3 rounded-lg border border-subtle inline-block tracking-wider">G{{ $c->gravida }} · P{{ $c->parity }}</div>
 </td>
 <td class="px-6 py-4 text-center">
 <div class="text-[12px] font-semibold text-slate-400 uppercase tracking-wider">{{ $c->gestational_age_weeks }} WEEKS</div>
 </td>
 <td class="px-6 py-4">
 @if($latest)
 <div class="space-y-1">
 ...
 <span class="text-[12px] font-semibold {{ $latest->fetal_heart_rate < 110 || $latest->fetal_heart_rate > 160 ? 'text-rose-500 animate-pulse' : 'text-emerald-500' }} ">
 {{ $latest->fetal_heart_rate }} BPM
 </span>
 </div>
 <div class="flex items-center gap-2">
 <span class="text-[8px] font-semibold text-slate-600 uppercase ">NODE:</span>
 <span class="text-[12px] font-semibold text-white ">{{ $latest->cervical_dilation_cm }} CM</span>
 </div>
 </div>
 @else
 ...
 <span class="text-[8px] font-semibold text-slate-600 uppercase ">{{ $dil >= 10 ? 'STAGE_II' : 'STAGE_I' }}</span>
 <span class="text-[12px] font-semibold text-slate-300 ">{{ $dil }}/10CM</span>
 </div>
