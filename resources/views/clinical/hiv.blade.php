...
<div class="space-y-8 animate-fade-in pb-20">
 ...
 <p class="text-[12px] font-semibold text-slate-500 font-medium mb-4 ">{{ $label }}</p>
 <p class="text-5xl font-semibold text-{{ $color === 'indigo' ? 'indigo-400' : ($color === 'amber' ? 'amber-500' : 'emerald-500') }} tracking-tighter leading-none">{{ $val }}</p>
 <p class="text-[12px] font-semibold text-slate-600 mt-6 font-medium ">Target Matrix: 95% {{ $sub }}</p>
 </div>
 @endforeach
 </div>
 ...
 <div class="flex gap-4">
 <span class="px-4 py-1.5 bg-[#2a2e38] border border-subtle text-slate-500 rounded-xl text-[12px] font-semibold uppercase ">{{ $metrics['counts']['on_art'] }} ACTIVE_ON_ART</span>
 </div>
 </div>
 ...
 <td class="px-10 py-6">
 <div class="font-semibold text-white text-base uppercase group-hover:text-sage transition-colors">{{ $e->full_name }}</div>
 <div class="text-[12px] font-bold text-slate-500 font-medium mt-1 ">AGE: {{ $e->age }} · {{ strtoupper($e->gender) }}</div>
 </td>
 <td class="py-6 font-mono text-[12px] font-semibold text-sage tracking-wider">{{ $e->art_number }}</td>
 <td class="py-6">
 <span class="px-3 py-1 bg-[#2a2e38] border border-subtle rounded-lg text-[12px] font-semibold text-slate-400 uppercase ">{{ $e->current_regimen ?: 'TREATMENT_PENDING' }}</span>
 @if($e->regimen_line)
 <div class="text-[8px] font-semibold text-slate-600 mt-2 font-medium ">{{ $e->regimen_line }} LINE_PROTOCOL</div>
 @endif
 </td>
 <td class="py-6 text-[12px] font-semibold text-slate-500 uppercase ">Stage {{ $e->who_stage }}</td>
 <td class="py-6">
 @if($e->last_vl !== null)
 <div class="text-sm font-semibold {{ $e->last_vl >= 1000 ? 'text-rose-500 animate-pulse' : 'text-emerald-500' }} tracking-tighter">
 {{ number_format($e->last_vl) }} COPIES/ML
 </div>
 <div class="text-[8px] font-semibold text-slate-600 uppercase mt-1 ">{{ date('d M Y', strtotime($e->last_vl_date)) }}</div>
 @else
 ...
 <button onclick="openVisitModal('{{ $e->id }}')" class="px-4 py-1.5 bg-sage/10 text-sage border border-indigo-500/20 rounded-lg text-[8px] font-semibold font-medium hover:bg-sage hover:text-white transition-all">Visit</button>
 <button onclick="openRegimenModal('{{ $e->id }}', '{{ $e->current_regimen }}')" class="px-4 py-1.5 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded-lg text-[8px] font-semibold font-medium hover:bg-amber-600 hover:text-white transition-all">Regimen</button>
 </div>
 </td>
 </tr>
 @empty
