...
<div class="space-y-8 pb-20">
 ...
 <div class="text-[8px] font-semibold text-slate-600 uppercase mt-1">{{ $c->source_unit ?? 'ORIGIN_NA' }}</div>
 </td>
 <td class="whitespace-nowrap px-6 py-4">
 @php $v = $c->latestVital; @endphp
 <div class="flex gap-4">
 <div class="text-center">
 <div class="text-[12px] font-semibold text-slate-200">{{ $v ? $v->bp_systolic.'/'.$v->bp_diastolic : '—' }}</div>
 <div class="text-[8px] font-bold text-slate-600 font-medium">BP</div>
 </div>
 <div class="text-center">
 <div class="text-[12px] font-semibold {{ ($v->spo2 ?? 100) < 90 ? 'text-rose-500 animate-pulse' : 'text-emerald-500' }}">
 {{ $v ? $v->spo2.'%' : '—' }}
 </div>
 ...
 <div class="text-[12px] font-semibold text-slate-400 mt-1 font-medium">Score: {{ $sofa }}</div>
 @else
...
<x-cc-modal id="admitModal" title="Critical Intake Authorization" icon="fa-hospital-user">
 <form method="POST" action="{{ route('specialty.critical.icu.admit') }}" class="space-y-6">
 @csrf
...
<x-cc-modal id="vitalsModal" title="Hemodynamic Surveillance Log" icon="fa-heart-pulse">
 <form method="POST" action="{{ route('specialty.critical.icu.vitals') }}" class="space-y-6">
 @csrf
 ...
 <input name="{{ $name }}" type="number" step="0.1" class="w-full bg-card/50 border border-subtle rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-200 outline-none">
 </div>
 @endforeach
 </div>
 <div class="pt-4">
...
<x-cc-modal id="sofaModal" title="SOFA Strategic Indexing" icon="fa-chart-simple">
 <form method="POST" action="{{ route('specialty.critical.icu.sofa') }}" class="space-y-6">
 @csrf
 ...
 <div>
 <label class="block text-[12px] font-semibold text-slate-500 font-medium mb-2">{{ $label }}</label>
 <select name="{{ $name }}" class="w-full bg-card/50 border border-subtle rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-200 outline-none">
...
function openDischargeModal(id) {
  let url = "{{ route('specialty.critical.icu.discharge', ':id') }}";
