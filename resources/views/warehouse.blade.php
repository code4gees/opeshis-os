...
<div class="max-w-7xl mx-auto space-y-8 animate-fade-in pb-20">
 ...
 <h3 class="text-3xl font-bold text-white">{{ number_format($stats['total_items']) }}</h3>
 </div>
 <div class="bg-[#2a2e38] rounded-xl p-6 border border-subtle ">
 ...
 <h3 class="text-3xl font-bold text-sage">{{ $stats['pending_reqs'] }}</h3>
 </div>
 <div class="bg-[#2a2e38] rounded-xl p-6 border border-subtle ">
 ...
 <div class="text-xl font-bold {{ $b->units_available < 10 ? 'text-rose-500' : 'text-white' }}">{{ $b->units_available }}</div>
 ...
 <td class="px-6 py-4">
 <span class="px-2 py-1 rounded text-[12px] font-bold {{ $a->movement_type === 'IN' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500' }}">
 {{ $a->movement_type }}
 </span>
 </td>
 ...
 <tr class="hover:bg-slate-700/30 transition-colors">
 <td class="px-6 py-4">
 <div class="text-white font-bold">{{ $s->item_name }}</div>
 <div class="text-[12px] text-slate-500 font-medium mt-0.5 uppercase">Batch: {{ $s->batch_number ?: 'N/A' }}</div>
 </td>
 ...
 <tr class="hover:bg-slate-700/30 transition-colors">
 <td class="px-6 py-4">
 <div class="text-white font-bold uppercase text-xs">{{ $r->pharmacy_item_name }}</div>
 <div class="text-[12px] text-slate-500 font-medium mt-0.5">QTY: {{ $r->requested_qty }}</div>
 </td>
 ...
 @elseif($r->status === 'dispatched') bg-sage/10 text-sage
 ...
 @if($r->status === 'pending')
 <button onclick="openDispatchModal('{{ $r->id }}', '{{ $r->pharmacy_item_name }}', {{ $r->requested_qty }})" class="px-4 py-2 bg-sage hover:bg-sage text-white text-[12px] font-bold rounded-lg transition-all">Approve Dispatch</button>
 @else
 ...
 <div class="text-[12px] text-slate-500 font-medium mt-0.5">Contact: {{ $v->contact_person ?? 'N/A' }}</div>
 </td>
 <td class="px-6 py-4 text-center text-slate-300 font-bold">{{ $v->item_count ?? 0 }}</td>
 <td class="px-6 py-4 text-center">
 <span class="px-2 py-0.5 rounded text-[12px] font-bold uppercase {{ ($v->status ?? 'active') === 'active' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500' }}">
 {{ $v->status ?? 'active' }}
 </span>
 </td>
 </tr>
 ...
 <form method="POST" action="{{ route('operations.supply.warehouse.action') }}" class="space-y-4">
 @csrf
...
<div id="addStockModal" class="fixed inset-0 bg-card/80 z-[100] hidden flex items-center justify-center p-6">
 ...
 <form method="POST" action="{{ route('operations.supply.warehouse.action') }}" class="space-y-5">
 @csrf
