...
@section('title', 'Financial Reconciliation — Opeshis OS')
        ...
        <div class="flex gap-6">
            <div class="flex items-center gap-8 px-10 py-4 glass-panel rounded-[2rem] border border-white/10 bg-white/[0.02]">
                <div class="text-center">
                    ...
                    <p class="text-3xl font-black text-white italic tracking-tighter leading-none">{{ $history->where('status', 'success')->count() }}</p>
                </div>
                ...
                <div class="text-center">
                    ...
                    <p class="text-3xl font-black text-white italic tracking-tighter leading-none">{{ $history->count() }}</p>
                </div>
            </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="hover:bg-white/5 transition-all group">
                        <td class="px-10 py-6">
                            <div class="font-black text-white text-base uppercase group-hover:text-emerald-400 transition-colors">{{ $h->reference_id }}</div>
                            <div class="text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em] mt-1 italic">Type: {{ strtoupper($h->transaction_type) }}</div>
                        </td>
                        <td class="px-6 py-6 text-center">
                            <div class="text-sm font-black text-white italic tracking-tighter">{{ $settings['currency_symbol'] ?? 'FCFA' }} {{ number_format($h->amount, 2) }}</div>
                        </td>
                        <td class="px-6 py-6">
                        ...
                        <td class="px-6 py-6">
                            ...
                            <div class="flex items-center gap-3">
                                <span class="w-2 h-2 rounded-full {{ $h->status === 'success' ? 'bg-emerald-500' : 'bg-rose-500' }} {{ $statusCls }}"></span>
                                <span class="text-[9px] font-black {{ $statusCls }} uppercase tracking-widest">
                                    {{ strtoupper($h->status) }}
                                </span>
                            </div>
                        </td>
                        <td class="px-10 py-6 text-right text-[10px] font-black text-slate-500 uppercase tracking-widest italic">
                            {{ \Carbon\Carbon::parse($h->created_at)->format('d M · H:i:s') }}
                        </td>
                    </tr>
                    @endforeach
                    @if($history->isEmpty())
...
<div id="syncModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] hidden flex items-center justify-center p-8">
    <div class="glass-panel w-full max-w-xl rounded-[3rem] p-12 shadow-2xl border border-white/10">
        ...
        <form method="POST" action="{{ url('/ops/finance/sync') }}" class="space-y-6">
            @csrf
            <div>
