...
<div class="space-y-8 animate-fade-in pb-20">
 ...
 <p class="text-3xl font-semibold text-white tracking-tighter leading-none">{{ $history->where('status', 'success')->count() }}</p>
 </div>
 <div class="w-px h-10 bg-white/10 "></div>
 <div class="text-center">
 <p class="text-[12px] font-semibold text-sage font-medium mb-2 ">Total Logs</p>
 <p class="text-3xl font-semibold text-white tracking-tighter leading-none">{{ $history->count() }}</p>
 </div>
 </div>
 ...
 <td class="px-10 py-6">
 <div class="font-semibold text-white text-base uppercase group-hover:text-emerald-400 transition-colors">{{ $h->reference_id }}</div>
 <div class="text-[12px] font-bold text-slate-500 font-medium mt-1 ">Type: {{ strtoupper($h->transaction_type) }}</div>
 </td>
 <td class="px-6 py-6 text-center">
 <div class="text-sm font-semibold text-white tracking-tighter">{{ $settings['currency_symbol'] ?? 'FCFA' }} {{ number_format($h->amount, 2) }}</div>
 </td>
 <td class="px-6 py-6">
 ...
 <span class="text-[12px] font-semibold {{ $statusCls }} font-medium">
 {{ strtoupper($h->status) }}
 </span>
 </div>
 </td>
