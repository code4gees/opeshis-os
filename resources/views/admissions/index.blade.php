        ...
        @foreach($wards as $ward)
            <div class="glass-panel rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/10 overflow-hidden">
                <div class="px-8 py-6 border-b border-white/5 flex justify-between items-center bg-slate-50/30">
                    <h3 class="text-[10px] font-black text-white uppercase tracking-widest">{{ $ward->name }} <span class="text-slate-300 ml-2">({{ $ward->type }})</span></h3>
                    <span class="px-3 py-1 glass-panel border border-white/10 rounded-lg text-[8px] font-black uppercase text-slate-400">{{ $ward->beds->where('status', 'Occupied')->count() }}/{{ $ward->beds->count() }} Beds</span>
                </div>
                <div class="p-6 grid grid-cols-4 gap-4">
                    @foreach($ward->beds as $bed)
                        <div class="aspect-square rounded-2xl border {{ $bed->status === 'Occupied' ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'bg-white/5 border-white/10 text-slate-300' }} flex flex-col items-center justify-center cursor-pointer group relative">
                            <span class="text-[10px] font-black">{{ $bed->bed_number }}</span>
                            @if($bed->status === 'Occupied')
                                <div class="absolute inset-0 bg-slate-900/90 rounded-2xl opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center p-2 text-center">
                                    <p class="text-[7px] font-black uppercase text-white truncate w-full">{{ $bed->full_name }}</p>
                                    <p class="text-[6px] font-black uppercase text-indigo-400 mt-1">{{ $bed->medical_id }}</p>
                                </div>
                            @endif
                        </div>
                    @endforeach
                ...
                <tbody class="divide-y divide-slate-50">
                    @foreach($activeAdmissions as $a)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-10 py-6">
                                <div class="font-black text-white text-sm">{{ $a->full_name }}</div>
                                <div class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">{{ $a->medical_id }}</div>
                            </td>
                            <td class="px-10 py-6">
                                <div class="text-[11px] font-black text-indigo-600 uppercase tracking-widest">{{ $a->ward_name }}</div>
                                <div class="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Bed: {{ $a->bed_number }}</div>
                            </td>
                            <td class="px-10 py-6">
                                <div class="text-[11px] text-slate-700 font-bold max-w-xs truncate">{{ $a->diagnosis_at_admission }}</div>
                                <div class="text-[9px] text-slate-400 font-black uppercase mt-1">{{ \Carbon\Carbon::parse($a->admission_date)->diffForHumans() }}</div>
                            </td>
